...
class WatchDraffService:
    ...
    def create_watch(self, watch_id: UNIQUEIDENTIFIER, seller_id: UNIQUEIDENTIFIER, name: str, brand: str, price: float, origin: str, model: str, produce_at , status: str, image: str, description: str, condition: str) -> WatchDraffModel:
        watch = WatchDraffModel(watch_id=watch_id, seller_id=seller_id, name=name, brand=brand, price=price, origin=origin, model=model, produce_at=produce_at, status=status, image=image, description=description, condition=condition)
